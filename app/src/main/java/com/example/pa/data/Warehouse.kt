...
class Warehouse {
    ...
    fun createProducts() {
        if (called == 0) {
            ...
            val imageListM = ArrayList<Int>()
            val reviewListM = ArrayList<Review>()
            imageListM.add(R.drawable.zyklon1)
            reviewListM.add(Review("Adolf Hitler", "Great poison. Small amounts of it kill people in minutes. Managed to kill 1.1 million people with it.", 8, LocalDate.of(1945, 4, 11)))
            reviewListM.add(Review("Sami Vincent", "Ok pesticide, but is banned in some countries.", 6, LocalDate.of(2021, 7, 4)))
            products.add(Product("Pharmacy", 7.40, "Zyklon B", "Degussa", "Hydrogen cyanide is a poisonous gas that interferes with cellular respiration. Cyanide prevents the cell from producing adenosine triphosphate (ATP) by binding to one of the proteins involved in the electron transport chain.", "Nazi Germany", LocalDate.of(2389, 4, 24), 5.0, 5, imageListM, reviewListM, 24329))
            val imageListN = ArrayList<Int>()
            val reviewListN = ArrayList<Review>()
            imageListN.add(R.drawable.vodka1)
            reviewListN.add(Review("Gerard DuGalle", "Your vodka can wait, my good Alexei.", 7, LocalDate.of(2500, 9, 29)))
            products.add(Product("Beverage", 23.75, "Vodka", "Brrrrr", "mmm alcohol", "Russia", LocalDate.of(2024, 8, 1), 1.3, 14, imageListN, reviewListN, 73))
